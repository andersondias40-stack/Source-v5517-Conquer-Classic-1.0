...
namespace COServer.Game.MsgTournaments
{
    public class KillerSystem
    {
        ...
        public void Update(Client.GameClient client)
        {
            ...
            if (UID == client.Player.UID && Flag < KillFlag.UnStoppable && Dead == false)
            {
                switch (Flag)
                {
        ...
        public void Send(Client.GameClient client)
        {
            ...
            using (var rec2 = new ServerSockets.RecycledPacket())
            {
                ...
                {
                    ID = MsgServer.MsgStringPacket.StringID.Sound,
                ...
                // Mensagem de texto
                var msg = new MsgMessage($"{this.Flag} {client.Player.Name}.",
                                         MsgMessage.MsgColor.red,
                                         MsgMessage.ChatMode.System);
                stream2 = msg.GetArray(stream2);

                foreach (var user in Database.Server.GamePoll.Values)
                {
                    if (user.Player.Map == client.Player.Map &&
                        user.Player.DynamicID == client.Player.DynamicID)
                    {
                        user.Send(stream);
                        user.Send(stream2);
                    }
